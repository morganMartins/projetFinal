...
describe('Action sur Trello', () => {
  ...
  // CAS_02_Tableau_Création d'un tableau
    ...
    cy.get('[data-testid="board-name-display"]').should('have.text', nomTableau)
  });
  ...
  it('CAS_08_Membre', () => {
    ...
    cy.wait(1000)
    cy.mailslurp()
      .then(mailslurp => mailslurp.waitForLatestEmail('94db1f4c-f721-4692-a411-847a622bdaef', 10000, true))
      .then(email => {
      ...
      cy.get('[data-testid="board-share-button"]').click()
      cy.get(':nth-child(2) > :nth-child(3) > [data-testid="board-permission-selector"] > div > [data-testid="board-permission-selector-dropdown--trigger"]').click()
      cy.get(':nth-child(2) > .css-1xg4o0').click()
      cy.wait(1000)
      cy.get('[data-testid="confirm-remove-deactived-member-button"]').click()
      cy.wait(1000)
      cy.reload()
      cy.wait(1000)     
      })
